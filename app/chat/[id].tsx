...
import { ArrowBendUpLeft, Check, Checks, Microphone, PaperPlaneTilt, Play, Pause, Trash, Users, X } from 'phosphor-react-native';
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, minWidth: 160 }}>
      ...
      <View style={{ flex: 1, position: 'relative', overflow: 'hidden' }} onLayout={(e) => setContainerWidth(e.nativeEvent.layout.width)}>
        {/* inactive bars — always visible */}
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
          {bars.map((h, i) => (
            <View key={i} style={{ width: 3, height: h, borderRadius: 2, backgroundColor: `${accent}40` }} />
          ))}
        </View>
        {/* active clip overlay — width driven by Reanimated on UI thread */}
...
function Bubble({
  ...
  return (
    <View style={[s.bubbleRow, isMe ? s.alignRight : s.alignLeft]}>
      {!isMe ? (
        <PressableScale scaleTo={0.9} onPress={() => !selectMode && onAvatarPress?.(message.authorId)}>
      ...
      <View style={s.bubbleCol}>
        ...
        >
          <View style={[
            ...
            isOptimistic ? s.bubbleOptimistic : null,
            ...
            {selectMode && isMe && selected ? (
              <View style={s.checkBadge}>
                <Check size={10} color={c.bg} weight="bold" />
              </View>
            ) : null}
          </View>
        </SwipeableBubble>
...
export default function ChatScreen() {
  ...
  const { messages, loading, error, send, removeMessages, threadId, othersReadAt, othersTyping, sendTyping } = useChat(matchId);
  ...
  return (
    <Screen edges={['top']}>
      <BackHeader
        title={selectMode ? `${selectedIds.size} seleccionados` : title}
          ...
          selectMode ? (
            ...
            <PressableScale scaleTo={0.9} onPress={() => setParticipantsOpen(true)}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                ...
                <Text variant="caption" color="textTertiary">{match.joinedPlayers.length + 1}</Text>
              </View>
            </PressableScale>
          ) : undefined
        }
      ...
      <Sheet visible={participantsOpen} onClose={() => setParticipantsOpen(false)} title="Participantes">
        <View style={{ gap: spacing.sm, paddingBottom: spacing.xl }}>
          {match?.organizer ? (
            <PressableScale scaleTo={0.97} onPress={() => { setParticipantsOpen(false); setTimeout(() => router.push(`/perfil/${match.organizer.id}`), 200); }} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingVertical: spacing.sm }}>
          ...
          {match?.joinedPlayers.map((p) => (
            <PressableScale key={p.id} scaleTo={0.97} onPress={() => { setParticipantsOpen(false); setTimeout(() => router.push(`/perfil/${p.id}`), 200); }} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingVertical: spacing.sm }}>
